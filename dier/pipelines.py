...
class DierPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('dier.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `dier`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM dier WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO dier (date, title, link, content)"
